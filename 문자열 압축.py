...
import unittest
s = "aabbaccc"
class Mix_member_Test(unittest.TestCase):
    def test_1(self):
        unittest.TestCase.assertEqual(self, "aabbaccc", 7)
        unittest.TestCase.assertEqual(self, "ababcdcdababcdcd", 9)
        unittest.TestCase.assertEqual(self, "abcabcdede", 8)
        unittest.TestCase.assertEqual(self, "abcabcabcabcdededededede", 14)
        unittest.TestCase.assertEqual(self, "xababcdcdababcdcd" ,17)
...
def solution_1(s):
    ...
    for size in range(1,len(s)+1):
        start = 0
        p = s[start:start+size]
        if p == s[start+size:start+(2*size)]:
            for j in range(start,len(s)+1):
                contraction_str[p] = contraction_str.get(p, 1) + 1
                start += size
