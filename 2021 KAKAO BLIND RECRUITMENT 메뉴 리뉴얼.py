...
def test():
    assert solution3(["ABCFG", "AC", "CDE", "ACDE", "BCFG", "ACDEH"], [2,3,4]) == ["AC", "ACDE", "BCFG", "CDE"]
    assert solution3(["ABCDE", "AB", "CD", "ADE", "XYZ", "XYZ", "ACD"], [2,3,5]) == ["ACD", "AD", "ADE", "CD", "XYZ"]
    assert solution3(["XYZ", "XWY", "WXA"], [2,3,4]) == ["WX", "XY"]



def solution1(orders, course):
    # {단품이름:[함께구입한 다른 단품이름(중복가능),,,],{},..}
    order_info = {}
    for order in orders:
        for item in order:
            other_items = order.replace(item,'')
            order_info.setdefault(item,[]).extend(other_items)

from itertools import combinations
def solution2(orders, course):
    items_set = set(''.join(orders))
    candi_dict = dict()

    for i in course:
        for candi in combinations(items_set,i):
            for order in orders:
                if all(k in order for k in candi):
                    candi_dict[candi] = candi_dict.setdefault(candi,0) + 1
    
    # candi_dict = {''.join(sorted(i)):v for i, v in candi_dict.items() if v > 1}
    # print(candi_dict)
    best_sell = dict(course)
    for i, v in candi_dict.items():
        ...

def solution3(orders, course):
    item_count = {}
    for i in ''.join(orders):
        item_count[i] = item_count.setdefault(i,0) + 1
