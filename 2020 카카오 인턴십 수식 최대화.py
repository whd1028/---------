...
def test():
    assert solution1("100-200*300-500+20") == 60420
    assert solution1("50*6-3*2") == 300
...
def solution1(expression):
    ...
    for operators in permutations(['*','-','+'], 3):
        expre = re.findall(r"[0-9]+|[*\-+]{1}",expression)
        for operator in operators:
            while operator in expre:
                op_index = expre.index(operator)
                new_num = eval(f"({expre[op_index-1]}){operator}({expre[op_index+1]})")
                del expre[expre.index(operator)-1:expre.index(operator)+2]
                expre.insert(op_index-1,new_num)
        answer = max(abs(*expre),answer)
    return answer
