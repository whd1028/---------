# 문제 설명
# OO 연구소는 [한 번에 K 칸을 앞으로 점프하거나, (현재까지 온 거리) x 2 에 해당하는 위치로 순간이동]을 할 수 있는 특수한 기능을 가진 아이언 슈트를 개발하여 판매하고 있습니다. 
...
# 단, 건전지 사용량을 줄이기 위해 [점프로 이동하는 것은 최소]로 하려고 합니다. 
# 아이언 슈트 구매자가 이동하려는 거리 N이 주어졌을 때, 사용해야 하는 [건전지 사용량의 최솟값을 return]하는 solution 함수를 만들어 주세요.
...
def test():
    ...
    assert solution2(6) == 2
    assert solution2(5000) == 5
...
def solution1(n):
    ...
    while p:
        ...
        for j in count(0):
            if i*(2**j) < n:
                teleport.append(i*(2**j))
            else:
                break
        move_info.append((i,n-teleport[-1]))
        move = min(move, i+(n-max(teleport)))
        p = p - set(teleport)
        i += 1
...
def solution2(n):
    ...
    return move+solution2(n)
    
def solution3(n): # 이건 생각지도 못 했네....
