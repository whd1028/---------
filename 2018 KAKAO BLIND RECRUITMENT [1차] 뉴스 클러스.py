...
def test():
    # assert solution1("FRANCE", "french") == 16384
    # assert solution1("handshake", "shake hands") == 65536
    # assert solution1("aa1+aa2", "AAAA12") == 43690
    assert solution1("E=M*C^2", "e=m*c^2") == 65536

from collections import Counter as ct
def solution1(a: str, b: str) -> int:
    a = a.lower()
    b = b.lower()
    ...
    for i in range(len(b)-1):
        if b[i].isalpha() and b[i+1].isalpha():
            b_ct[f"{b[i]}{b[i+1]}"] += 1

    inter = len(list((a_ct&b_ct).elements()))
    union = len(list((a_ct|b_ct).elements()))
    if inter == 0 and union == 0:
        J = 1*65536
    else:
        J = int(inter/union*65536)
